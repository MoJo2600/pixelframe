...
uint8_t VisualsFrame::attackDecayWave8( uint8_t i)
{
  if( i < 86) {
    return i * 3;
  } else {
...
void VisualsFrame::coolLikeIncandescent( CRGB& c, uint8_t phase)
{
  if( phase < 128) return;
...
CRGB VisualsFrame::computeOneTwinkle( uint32_t ms, uint8_t salt)
{
  ...
  if( ((slowcycle8 & 0x0E)/2) < TWINKLE_DENSITY) {
    bright = this->attackDecayWave8( fastcycle8);
  }
...
void VisualsFrame::drawTwinkles( )
{
  ...
  for ( uint32_t idx=0; idx < 256; idx++) {
    ...
    CRGB c = this->computeOneTwinkle( myclock30, myunique8);
...
void VisualsFrame::chooseNextColorPalette( CRGBPalette16& pal)
{
